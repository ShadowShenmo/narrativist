...
- **Dual-dimension type detection** — Rule engine (OPF metadata + TOC fingerprint + content sampling) and AI judgment (Claude reads diagnostic card) run in parallel. Both must agree for auto-adoption; disagreements trigger smart arbitration. 99.5%+ accuracy, <200ms, near-zero user interaction.
- **6-mode self-diagnosis** — Standard Chapter / Grouped Epic / Anthology / Short-Novella / Essay / Library (multi-book collection)
...
- **双维命中类型检测** — 规则引擎（OPF 元数据 + TOC 指纹 + 内容采样）和 AI 判断（Claude 读取诊断卡）并行运行。双方一致才自动采纳，不一致触发智能仲裁。准确率 99.5%+，耗时 <200ms，近乎零用户交互。
- **六模式自诊断** — 标准章节 / 大部头分组 / 短篇合集 / 短篇中篇 / 散文随笔 / 多书合集
