...
- **4-layer type detection** — web search → TOC analysis → preface scanning → user confirmation. Determines book type (novel / novella / anthology / collected works) before reading begins
...
- **四层类型检测** — 网络搜索 → 目录结构分析 → 序言关键词检测 → 用户确认。阅读开始前必须确定作品类型
